...
type
  ...
  bins = array[0..length , 0..binMax] of integer;
{ binSorting }
var intTemp:integer;
...
function getBinYHeight(bin: bins; i: integer{i=x pos}) : integer; begin
    countera:=0;
    suma:=0;
    while countera<=binMax do begin
...
var
  ...
  timeTaken: Comp;
  dateTimeLoc, dateTimeEnd: TTimeStamp;
begin
  // get time to benchmark
  dateTimeLoc:=DateTimeToTimeStamp(Time());
  randomize();
  // get dataset
  dataSet:=randDataSet();
  // init bin
  ...
  while counterX <= length do begin
      counterY:=0;
      while counterY <= binMax do begin
  ...
  while dataSetCounter<=length do begin
      ...
      while binCounterX<=length do begin
          if (getBinSum(bin,binCounterX) + dataSet[dataSetCounter]) <= binMax then begin
  ...
  dateTimeEnd:=DateTimeToTimeStamp(Time());
  timeTaken:=(TimeStampToMSecs(dateTimeEnd) - TimeStampToMSecs(dateTimeLoc));
  writeln(timeTaken:10:5);
  // stop program loop
